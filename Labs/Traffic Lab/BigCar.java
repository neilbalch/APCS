...
public class BigCar extends Automobile {
   public BigCar(Color bodyColor, Point position) {
      ...
      setDimensions(new Dimension(40, 15));
   }
   ...
   public void drawMe(Graphics g) {
      ...
      g.fillOval(getPosition().x + 1,  getPosition().y + 10,   wheel_diameter,   wheel_diameter);
      g.fillOval(getPosition().x + 20, getPosition().y + 10,   wheel_diameter,   wheel_diameter);

      // Hubs
      g.setColor(Color.LIGHT_GRAY);
      int hub_diameter = 10;
      g.fillOval(getPosition().x + 5,  getPosition().y + 13,   hub_diameter,  hub_diameter);
      g.fillOval(getPosition().x + 24, getPosition().y + 13,   hub_diameter,  hub_diameter);
